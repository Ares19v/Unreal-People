...
export default function Home() {
  ...
  const AuthModal = () => (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.9)", backdropFilter: "blur(20px)", zIndex: 1200, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "Inter, sans-serif" }}>
      <div style={{ width: "400px", padding: "60px", background: colors.bg, border: "1px solid " + colors.line, position: "relative" }}>
        <button onClick={() => setShowAuth(false)} style={{ position: "absolute", top: "20px", right: "20px", background: "none", border: "none", color: colors.sub, cursor: "pointer", fontSize: "10px" }}>[X]</button>
        <h2 style={{ fontSize: "12px", fontWeight: "900", letterSpacing: "5px", marginBottom: "40px", textAlign: "center", color: colors.text }}>INITIALIZE_ID</h2>
        <input placeholder="EMAIL" style={{ width: "100%", background: "none", border: "none", borderBottom: "1px solid " + colors.line, color: colors.text, padding: "10px 0", marginBottom: "20px", outline: "none", fontSize: "11px" }} />
        <input type="password" placeholder="KEY" style={{ width: "100%", background: "none", border: "none", borderBottom: "1px solid " + colors.line, color: colors.text, padding: "10px 0", marginBottom: "40px", outline: "none", fontSize: "11px" }} />
        <button style={{ width: "100%", background: colors.text, color: colors.bg, border: "none", padding: "15px", fontSize: "10px", fontWeight: "900", cursor: "pointer" }}>AUTHENTICATE</button>
      </div>
    </div>
  );
  ...
  const EditModal = () => (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.9)", backdropFilter: "blur(20px)", zIndex: 1100, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "Inter, sans-serif" }}>
      <div style={{ width: "500px", padding: "50px", background: colors.bg, border: "1px solid " + colors.line }}>
        ...
        <label style={{ fontSize: "9px", fontWeight: "900", color: colors.sub, display: "block", marginBottom: "10px" }}>ICON</label>
        <div style={{ display: "flex", gap: "20px", marginBottom: "25px" }}>
          {Object.keys(Icons).map((type) => {
            const IconItem = Icons[type as keyof typeof Icons];
            return (
              <button key={type} onClick={() => updateAgentDetail(editingAgent.id, "type", type)} style={{ background: editingAgent.type === type ? colors.line : "none", border: "1px solid " + (editingAgent.type === type ? colors.text : "transparent"), padding: "10px", cursor: "pointer", color: colors.text }}>
                <IconItem />
              </button>
            );
          })}
        </div>
        <label style={{ fontSize: "9px", fontWeight: "900", color: colors.sub, display: "block", marginBottom: "10px" }}>ISOLATED_FONT</label>
        <div style={{ display: "flex", gap: "10px", marginBottom: "35px" }}>
          <button onClick={() => updateAgentDetail(editingAgent.id, "font", "SANS")} style={{ flex: 1, padding: "12px", background: editingAgent.font === "SANS" ? colors.text : "none", color: editingAgent.font === "SANS" ? colors.bg : colors.text, border: "1px solid " + colors.line, fontSize: "10px", fontWeight: "900", cursor: "pointer" }}>SANS</button>
          <button onClick={() => updateAgentDetail(editingAgent.id, "MONO")} style={{ flex: 1, padding: "12px", background: editingAgent.font === "MONO" ? colors.text : "none", color: editingAgent.font === "MONO" ? colors.bg : colors.text, border: "1px solid " + colors.line, fontSize: "10px", fontWeight: "900", cursor: "pointer" }}>MONO</button>
        </div>
  ...
  return (
    <main style={{ background: colors.bg, color: colors.text, minHeight: "100vh", display: "flex", fontFamily: "Inter, sans-serif" }}>
      {editingAgent && <EditModal />} {showAuth && <AuthModal />}
